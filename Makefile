...
run: build
	$(BUILD_DIR)/$(BINARY_NAME) -org hashicorp -repo terraform-provider-azurerm

run-with-token: build
	@test -n "$(GITHUB_TOKEN)" || (echo "Error: GITHUB_TOKEN not set" && exit 1)
	$(BUILD_DIR)/$(BINARY_NAME) -org hashicorp -repo terraform-provider-azurerm -token $(GITHUB_TOKEN)

run-custom: build
	$(BUILD_DIR)/$(BINARY_NAME) -org $(ORG) -repo $(REPO) -db $(DB_PATH)
